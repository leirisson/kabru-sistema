        ...
        <div className="mx-auto flex max-w-screen-2xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#FFD700] text-[#0A0A0A] font-bold text-lg shadow-md">
                K
              </div>
              <span className="text-xl font-bold text-[#0A0A0A] dark:text-slate-100">Kabru Sistema</span>
            </div>
            <nav className="flex gap-1">
              ...
              {(session.role === 'ADMIN' || session.role === 'FATURAMENTO' || session.role === 'VENDEDOR') && (
                <Link
                  href="/financeiro"
                  className="flex items-center gap-2 rounded-lg px-4 py-2.5 text-xl font-medium text-slate-700 hover:bg-[#FFD700]/10 hover:text-[#0A0A0A] transition-colors dark:text-slate-300 dark:hover:text-white dark:hover:bg-[#FFD700]/20"
                >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3 text-sm">
              <div className="flex items-center gap-3 rounded-full bg-slate-100 px-3 py-2 dark:bg-slate-700">
                ...
                <div className="hidden sm:block">
                  <p className="text-sm font-semibold text-[#0A0A0A] dark:text-slate-100">{session.nome}</p>
                  <span className="text-xs font-medium text-slate-500 dark:text-slate-400">{LABEL_ROLE[session.role]}</span>
                </div>
              </div>
            </div>
            <form action={logout}>
